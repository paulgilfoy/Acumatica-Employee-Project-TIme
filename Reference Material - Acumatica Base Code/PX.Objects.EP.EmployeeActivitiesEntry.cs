...
public class EmployeeActivitiesEntry : PXGraph<EmployeeActivitiesEntry>
	{
		...
		public EmployeeActivitiesEntry()
		{
			...
			{
				string currentScreen = PXContext.GetScreenID();
				if (!ScreenIDsEqual(currentScreen, PXSiteMap.Provider.FindSiteMapNode(typeof(SYImportMaint))?.ScreenID)
					&& !ScreenIDsEqual(currentScreen, PXSiteMap.Provider.FindSiteMapNode(typeof(SYExportMaint))?.ScreenID))
				{
		...
		protected virtual void _(Events.RowUpdated<EPActivityApprove> e)
		{
			if (e.Row.Date.GetValueOrDefault() != e.OldRow.Date.GetValueOrDefault()
					...
					|| e.Row.ShiftID.GetValueOrDefault() != e.OldRow.ShiftID.GetValueOrDefault())
			{
				EPEmployee employee = PXSelect<EPEmployee, Where<EPEmployee.defContactID, Equal<Required<EPActivityApprove.ownerID>>>>.Select(this, e.Row.OwnerID);
				if (employee != null)
				{
					var cost = CostEngine.CalculateEmployeeCost(null, e.Row.EarningTypeID, e.Row.LabourItemID, e.Row.ProjectID, e.Row.ProjectTaskID, e.Row.CertifiedJob, e.Row.UnionID, employee.BAccountID, e.Row.Date.Value, e.Row.ShiftID);
		...
		protected virtual void EPActivityApprove_EarningTypeID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			...
			EPEmployee rowEmploye = PXSelect<EPEmployee>.Search<EPEmployee.defContactID>(this, Filter.Current.OwnerID);
		...
		private bool ScreenIDsEqual(string id1, string id2)
		{
			...
			return id1.Replace(".", "").Equals(id2.Replace(".", ""), StringComparison.OrdinalIgnoreCase);
		}

		//Implementation class for CrewTimeEntryGraph generic extension.
		public class CrewTimeEntry : CrewTimeEntryGraph<EmployeeActivitiesEntry, PMTimeActivityFilter> { }
	}