...
public class EmployeeActivitiesRelease : PXGraph<EmployeeActivitiesRelease>
	{
		...
		public PXFilteredProcessingJoin<
			...
			InnerJoin<EPEmployee,
				On<EPEmployee.defContactID, Equal<EPActivityApprove.ownerID>,
		...
		public class EPActivityFilter : CR.OwnedFilter
		{
			...
			[TM.SubordinateAndWingmenOwnerEmployee(DisplayName = "Employee")]
		...
		protected static void ReleaseActivities(List<EPActivityApprove> activities)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (!contractUsageError && !costTransError)
				{
					ts.Complete();
				}
			}
        ...
        public static bool RecordCostTrans(RegisterEntry registerEntry, List<EPActivityApprove> activities, out bool activityAdded)
        {
            ...
            for (int i = 0; i < activities.Count; i++)
            {
                ...
                try
                {
					...
					if (epsetup != null)
					{
						EPEmployee employee = PXSelect<EPEmployee>.Search<EPEmployee.defContactID>(registerEntry, activity.OwnerID);
					var postingOptions = EPSetupMaint.GetPostingOption(registerEntry, epsetup, employee.BAccountID);
					if (activity.ProjectTaskID != null && postingOptions != EPPostOptions.DoNotPost)//cost transactions are created only if project is set.
					{
						...
						var cost = costEngine.CalculateEmployeeCost(activity.TimeCardCD, activity.EarningTypeID, activity.LabourItemID, activity.ProjectID, activity.ProjectTaskID, activity.CertifiedJob, activity.UnionID, employee.BAccountID, activity.Date.Value, activity.ShiftID);
						if (cost == null)
						{
							throw new PXException(Messages.EmployeeCostRateNotFound);
						}
						if (EPSetupMaint.GetPostPMTransaction(registerEntry, epsetup, employee.BAccountID))
							registerEntry.CreateTransaction(new RegisterEntry.CreatePMTran(activity, employee.BAccountID, activity.Date.Value, activity.TimeSpent, activity.TimeBillable, cost.Rate, cost.OvertimeMultiplier, cost.CuryID, true));
		...
		public static PMTran CreateContractUsage(RegisterEntry graph, int? contractID, PMTimeActivity timeActivity, int billableMinutes)
		{
			...
			if (laborItemID == null)
			{
				EPEmployee employeeSettings = PXSelect<EPEmployee,
					Where<EPEmployee.defContactID, Equal<Required<PMTimeActivity.ownerID>>>>.Select(graph, timeActivity.OwnerID);
			...
			EPContractRate matrix =
				PXSelectJoin<
					...
					Where<EPContractRate.contractID, Equal<Required<EPContractRate.contractID>>,
						...
						And<EPEmployee.defContactID, Equal<Required<CRPMTimeActivity.ownerID>>>>>>.Select(graph, contractID, activity.EarningTypeID, activity.OwnerID);
			if (matrix == null)
				matrix =
					PXSelect<
						EPContractRate,
